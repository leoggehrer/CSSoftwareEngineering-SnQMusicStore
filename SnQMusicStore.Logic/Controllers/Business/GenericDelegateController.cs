...
namespace SnQMusicStore.Logic.Controllers.Business
{
#if ACCOUNT_ON
        ...
        where TSourceEntity : Entities.IdentityEntity, TSourceContract, Contracts.ICopyable<TSourceContract>, new()
    {
        ...
        #region Query
        internal override async ValueTask<TEntity?> ExecuteGetEntityByIdAsync(int id)
        {
            var entity = await SourceEntityController.ExecuteGetEntityByIdAsync(id).ConfigureAwait(false);

            return entity != null ? ConvertTo(entity) : default;
        }
